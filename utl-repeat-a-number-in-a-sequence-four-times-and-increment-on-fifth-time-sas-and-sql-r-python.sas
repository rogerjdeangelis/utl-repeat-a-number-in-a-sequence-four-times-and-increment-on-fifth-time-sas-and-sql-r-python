...
s= \
"with recursive sequence(n) as ( select &tym1 union all select n + &step from sequence where n < (&tym2 + 1) ) select CAST(n AS INTEGER) as sequence from sequence where n < (&tym2+1)"
want=pdsql(s)
print(want);
...
%let step=0.25;                                                                  2001.00      2001 repeat
                          ...
                          want<-as.data.frame(rep(2000:2003, each = 4))
                          ...
                          data seq;
                            do seq=2001 to 2004 by .25 while (seq<2004);
                              sequence=floor(seq);
                              output;
                            end;
                          run;quit;
...
run;quit;
             ...
             does aqldf have a floor function
